...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if cfg_scale is not None and "perturbation" in model_kwargs:
            uncond_kwargs = dict(model_kwargs)
            null_index = _resolve_null_perturbation_index(model)
            uncond_kwargs["perturbation"] = null_index * th.ones_like(model_kwargs["perturbation"])
            uncond = model(x, self._scale_timesteps(t), **uncond_kwargs)
            model_output = (1 + cfg_scale) * model_output - cfg_scale * uncond

        if self.model_var_type in {ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE}:
            assert model_output.shape == (batch_size, channels * 2, *x.shape[2:])
        ...
        def process_xstart(x_start_prediction):
    ...
    def p_sample(self, model, x, t, clip_denoised=False, denoised_fn=None, cond_fn=None, model_kwargs=None, cfg_scale=None):
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    result = th.from_numpy(arr).to(device=timesteps.device)[timesteps].float()
    while len(result.shape) < len(broadcast_shape):
        result = result[..., None]
    return result.expand(broadcast_shape)


def _resolve_null_perturbation_index(model):
    current = model
    visited = set()
    while current is not None and id(current) not in visited:
        visited.add(id(current))
        if hasattr(current, "null_perturbation_index"):
            return getattr(current, "null_perturbation_index")
        ...
        if hasattr(current, "model"):
            ...
            continue
        current = None
    raise AttributeError("Could not resolve `null_perturbation_index` from wrapped model.")
