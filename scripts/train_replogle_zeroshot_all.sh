...
CONFIG_ROOT="${PROJECT_ROOT}/configs/replogle_zeroshot/runs"
...
run_single() {
  ...
  echo "[PerturbNova] launching zeroshot split=${split} config=${config_path}"
...
run_parallel_single_gpu() {
  ...
  for gpu_id in 0 1 2 3; do
    ...
    echo "[PerturbNova] launching zeroshot split=${split} on GPU ${gpu_id}"
    (
