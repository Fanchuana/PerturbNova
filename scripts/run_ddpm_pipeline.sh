#!/bin/bash
# DDPM Frozen Autoencoder Pipeline: Training + Inference + Cell Eval
#
# Usage:
#   bash scripts/run_ddpm_pipeline.sh vae
#   bash scripts/run_ddpm_pipeline.sh evoformer
#
# This script:
# 1. Trains DDPM with frozen autoencoder (4 GPUs)
# 2. Runs inference to generate predictions
# 3. Runs cell_eval on CPU (releases GPU)
...
if [[ "$MODEL_TYPE" == "vae" ]]; then
  TRAIN_CONFIG="configs/replogle_zeroshot/runs/hepg2/ddpm_frozen_vae_500k.toml"
  ...
  TRAIN_CONFIG="configs/replogle_zeroshot/runs/hepg2/ddpm_frozen_evoformer_500k.toml"
...
# Create inference config if it doesn't exist
if [[ ! -f "$INFER_CONFIG" ]]; then
  echo "Creating inference config..."
  mkdir -p "$(dirname "$INFER_CONFIG")"

  cat > "$INFER_CONFIG" << EOF
base_configs = ["../base.toml"]

[experiment]
name = "ddpm_frozen_${MODEL_TYPE}_zeroshot_hepg2_infer"
output_dir = "${OUTPUT_DIR}/inference"

[checkpoint]
path = "${OUTPUT_DIR}/checkpoints/latest.pt"
use_ema = false

[input.split]
dataset_config_path = "../../../replogle/data/source/zeroshot_hepg2.toml"

[output]
prediction_path = "${OUTPUT_DIR}/inference/predictions.h5ad"

[cell_eval]
outdir = "${OUTPUT_DIR}/inference/cell_eval"
